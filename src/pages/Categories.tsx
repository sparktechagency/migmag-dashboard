...
type TabType = "Genre" | "Key" | "License" | "Type";
...
const Categories: React.FC = () => {
  ...
  const handleDeletekeys = (id: number) => {
    Swal.fire({
      title: "Are you sure?",
      text: "You won't be able to revert this!",
      icon: "warning",
  ...
  const CatagoryModal = () => {
    ...
    const onFinish = async (values: any) => {
      if (activeTab === "Genre") {
        ...
        try {
          const res = await keyPost(infoKey).unwrap();
          if (res?.success === true) {
            toast.success(res.message);
            refetch();
          }
        ...
        try {
          const res = await licensePost(infoLicense).unwrap();
          if (res?.success === true) {
            toast.success(res.message);
            refetch();
          }
        ...
        try {
          const res = await typePost(infoType).unwrap();
          if (res?.success === true) {
            toast.success(res.message);
            refetch();
          }
  ...
  return (
    <div className="p-4">
      ...
      <div className="">
        <Tabs
          onSelect={(index) =>
            setActiveTab(["Genre", "Key", "License", "Type"][index] as TabType)
          }
        >
              ...
              <div className="flex flex-wrap gap-3">
                {keys?.data?.map((item: GenreItem) => (
                  ...
                  >
                    ...
                    <button onClick={() => handleDeletekeys(item.id)}>
                      <svg
                        width="24"
                        height="24"
                        viewBox="0 0 14 15"
          ...
          <TabPanel>
            <div className="bg-white rounded-2xl p-4">
              <div className="flex flex-wrap gap-3">
                {licens?.data?.map((item: GenreItem) => (
                  <div
                    key={item.id}
                    className="bg-[#f5f5f5] flex items-center justify-center gap-3 py-3  px-6 rounded-lg "
                  >
                    ...
                    <button onClick={() => handleDeleteLicense(item.id)}>
                      <svg
                        width="24"
                        height="24"
                        viewBox="0 0 14 15"
              ...
              <div className="flex flex-wrap gap-3">
                {type?.data?.map((item: GenreItem) => (
                  ...
                  >
                    ...
                    <button onClick={() => handleDeleteType(item.id)}>
                      <svg
                        width="24"
                        height="24"
                        viewBox="0 0 14 15"
